...
class MbAdvManager extends CI_Model {
	private $_TABLE_NAME = 'mb_adv';
	...
	public function select($model) {
		...
		/*else 
		if ($model->getHome()==1) {
			$sql = "SELECT * FROM ".$this->_TABLE_NAME." WHERE home_=1 ORDER BY order_";
		}
		else 
		if ($model->getRaovat()==1) {
			$sql = "SELECT * FROM ".$this->_TABLE_NAME." WHERE raovat_=1 ORDER BY order_";
		}*/		
		else {
	...
	protected function convert ($record) {
		...
		$model->setRaovat($record->raovat_);
		return $model;
	}	
}
