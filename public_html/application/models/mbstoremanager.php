...
class MbStoreManager extends CI_Model {
	private $_TABLE_NAME = 'mb_store';
	...
	public function config($model) {
		return $this->_config($this->_TABLE_NAME, $model);
	}	
	
	public function _config($table, $model) {
		$this->db->where('id_', $model->getId());
		$this->db->update($table, $this->getDataObjectConfig($model));		
	}
	...
	/**
	 * convert from Database Record to MbStoreModel
	 * @param record $record
	 * @return MbStoreModel
	 */
	protected function convert ($record) {
		$model = new MbStoreModel();
		$model->setId($record->id_);
		$model->setName($record->name_);
		$model->setOwner($record->owner_);
		$model->setDescription($record->description_);
		$model->setIntroduction($record->introduction_);
		$model->setPolicy($record->policy_);
		$model->setContact($record->contact_);
		$model->setMapLatitude($record->map_latitude_);
		$model->setMapLongitude($record->map_longitude_);
	 ...
	 * @param MbStoreModel $model
	 * array
	 */
	protected function getDataObject($model) {
		return array(
			'name_' => $model->getName(),
			'owner_' => $model->getOwner(),
	...
	protected function getDataObjectConfig($model) { 
		return array(
			'introduction_' => $model->getIntroduction(),
			'policy_' => $model->getPolicy(),
			'contact_' => $model->getContact(),
			'skin_' => $model->getSkin(),
			'cid_' => $model->getCatId()
		);
	}
	...
	protected function getWhereObject($model) {
		...
		if (!is_null($model->getOwner())) {
			$whereCondition['owner_'] = $model->getOwner();
		}
