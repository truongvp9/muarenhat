...
class MbTinTucManager extends CI_Model {
	...
	public function select($model) {
		if ($model->getId()!=0) {
			$sql = "SELECT * FROM ".$this->_TABLE_NAME." WHERE id_=".$model->getId()." ORDER BY date_ DESC LIMIT 10";
		}
		else
		if ($model->getCat()!=0) {
			$sql = "SELECT * FROM ".$this->_TABLE_NAME." WHERE cat_=".$model->getCat()." ORDER BY date_ DESC LIMIT 10";
		}
		else {
			$sql = "SELECT * FROM ".$this->_TABLE_NAME." ORDER BY date_ DESC LIMIT 10";
		}
