...
class MbNeedManager extends CI_Model {
	private $_TABLE_NAME = 'mb_need';
	...
	public function select($cat_id='') {
		if (isset($cat_id) && $cat_id!='')
			$sql = "SELECT * FROM ".$this->_TABLE_NAME." WHERE cat_id_=".$cat_id." ORDER BY need_";
		else 
			$sql = "SELECT * FROM ".$this->_TABLE_NAME." ORDER BY need_";
	...
	/**
	 * convert from Database Record to MbNeedModel
	 * @param record $record
	 * @return MbNeedModel
	 */
	
	protected function convert ($record) {
		$model = new MbNeedModel();
		$model->setId($record->id_);
		$model->setNeed($record->need_);
