...
class Sanpham extends CI_Controller {
	...
	*/
	...
	public function do_add_new_product() {	
        ...
        include 'mailer.php';
        $subject = 'San pham moi muarenhat.net - '.$name;
        $msg  = 'Noi dung tin: <br>';
        $msg .= 'Ten san pham: '.$name.'<br>';
        $msg .= 'Gia:'.$price.'<br>';
        $msg .= 'Mo ta sp:'.$description.'<br>';
        $msg .= 'Thong so ky thuat:'.$technicalDescription.'<br>';
        $msg .= 'Ngay dang:'.$date_create;
        // send mail to sonlocdien@gmail.com 
        smtpmailer('truongnguyensmart@gmail.com', 'muarenhat.mailer@gmail.com', 'Mua Re Nhat', $subject, $msg);
	...
	public function createthumbnail2() {
	...
	if($_FILES['upload']['type'] == "image/jpeg" || $_FILES['upload']['type'] == "image/png" || $_FILES['upload']['type'] == "image/gif")
	{
        ...
        $image_default = $target_path. $date.basename( $_FILES['upload']['name']);
        
        $length = intval(strlen($image_default))-4;
        
        $new_name = substr($image_default, 0,$length);

        $target_path = $target_path .$date. basename( $_FILES['upload']['name']); 
		...
		if(move_uploaded_file($_FILES['upload']['tmp_name'], $target_path)) 
		{
			...
			$thumbnail = $target_folder.$new_name."_thumbnail.jpg"; // Set the thumbnail name
			...
			//150; // This can be a set value or a percentage of original size ($width)
	...
	public function edit($pid) {
		...
		$sql = "SELECT p.store_cat_id,p.id_,p.name_,p.price_,p.image_default_,p.image_thumbnail_,p.description_,p.technical_description_,p.user_id_,p.category_ FROM mb_product p inner join mb_category c on c.id_=p.category_ WHERE p.id_=".$pid;
		$query = $this->db->query($sql);
		$result = $query->result();
		if (count($result)>0) {			
			$product = $result[0]; 
			$cat_id = $product->category_;
			$data['cat_id'] = $cat_id;
			$data['p_id'] = $product->id_;
			$data['breadcrumb'] = $categoryManager->selectBreadcrumb($cat_id);		
			$data['product']=$product;	
			$user_id = $product->user_id_;
			$sql = "SELECT id_ FROM mb_store WHERE owner_=".$user_id;
			$query = $this->db->query($sql);
			$result = $query->result();	
	...
	public function dangsanpham($step=0,$cat_id=0) {
		...
		if ($active) {	
		if ($step==0) {			
			$this->load->view("select_category.html",$data);
		}
		else
		if ($step==1) {
			$this->load->view("select_category.html",$data);
			...
			echo "Ban hay chon danh muc san pham!";
		}		
		}// is active
		else { // not active
			// echo "Bạn chưa đăng ký gian hàng, kích vào đây để đăng ký gian hàng!";
			$this->load->library('session');
			$account = $this->session->userdata('account');
