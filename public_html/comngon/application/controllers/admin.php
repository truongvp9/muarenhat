	...
	*/
	...
	public function siteinfo() {
		if (isset($_POST['footer'])) {
			$footer = $_POST['footer']; //echo $footer; die;
			$msg = $_POST['msg'];
			$sql = "UPDATE mb_sys SET footer_address='".addslashes($footer)."'";
			$this->db->query($sql);	
			$sql = "UPDATE mb_sys SET footer_msg='".addslashes($msg)."'";
			//die($sql);
			$this->db->query($sql);	
	...
	public function removesupport($id) {
		$this->load->model('mbsupportmanager');
		$this->load->model('mbsupportmodel');		
		$supportManager = new MbSupportManager();
		$supportModel = new MbSupportModel();
		$supportModel->setId($id);
		$supportManager->delete($supportModel);
	...
	public function support($id=0) {
		...
		$supportManager = new MbSupportManager();
		$supportModel = new MbSupportModel();
		$quangcao = $supportManager->select($supportModel);
		$id = intval($id);
		if ($id!=0) {
			$supportModel->setId($id);
			$qcinfo = $supportManager->select($supportModel);
		}
		else {
			$qcinfo[0]= new MbSupportModel();
		}
		$this->load->view("admin/support.html",array('quangcao'=>$quangcao,"qcinfo"=>$qcinfo[0]));
	}	
	...
	public function newmenu() {
		...
		$menuModel = new MbMenuModel();
		$menuManager = new MbMenuManager();
		$menuModel->setName($title);
		$menuModel->setDetail($content);
		$menuModel->setImage($image_file_src);
		$id = $_POST['id'];
		if ($id=='') {
			$menuManager->insert($menuModel);
		}
		else {
			$menuModel->setId($id);
			$menuManager->update($menuModel);
		}
		redirect(site_url('admin/menu'));						
	}	
	
	/**
	 * dang ho tro truc tuyen
	 * 
	 */
	
	public function dangsupport() {
		$this->load->model("mbsupportmodel");
		$this->load->model("mbsupportmanager");
		$name = $_POST['name'];
		$mobile = $_POST['mobile'];
		...
		$supportManager = new MbSupportManager();
		$supportModel = new MbSupportModel();
		$supportModel->setName($name);
		$supportModel->setMobile($mobile);
		$supportModel->setYahoo($yahoo);
		$id = $_POST['id'];
		if ($id=='') {
			$supportManager->insert($supportModel);
		}
		else {
			$supportModel->setId($id);
			$supportManager->update($supportModel);
		}
		redirect(site_url('admin/support'));
	}	
	...
	public function menu($id=0) {
		...
		if ($id!=0) {
			$menuModel->setId($id);
			$menuinfo = $menuManager->select($menuModel);
		}
		else {
			$menuinfo[0]= new MbMenuModel();
		}		
			
		$data['content'] = $menuinfo;
		
		$tin = $menuManager->select($menuModel);
		
		$data['tin'] = $tin;
				
		$this->load->view("admin/thucdon.html",$data);
